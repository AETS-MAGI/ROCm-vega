...
| `GemmFwd1x1_0_1_int8` | GEMM-style INT8 solver | **未観測** + **IsApplicable 除外** + **runtime failure**（driver route） / **solution query success**（`y=int32` 直叩き） | `MIOpenDriver convint8` では自然選択・`-s 1` が `ConvDirectNaiveConvFwd` に留まり、`-S GemmFwd1x1_0_1_int8` と only-solver search は `Not applicable`。一方で direct API query では `x=int8, w=int8, y=int32` 条件なら `solution_id = 89` が見え、workspace query も通る | `gemm.cpp` の `IsApplicable()` 条件（code_verified）/ `vega64_int8_gemmcand_nat_1x1_n32_c64_k64_20260318.log` / `vega64_int8_gemmcand_force_1x1_n32_c64_k64_20260318.log` / `vega64_int8_gemmcand_onlysolver_search_1x1_n32_c64_k64_20260318.log` / `vega64_miopen_int8_solution_probe_20260318.log`（runtime_verified / query_verified） | **実装 / 適用条件制約寄り**。少なくとも current installed path では descriptor / output-type 境界が絡む | AMD(M) | installed binary は host 上で `miopen-hip 7.2.0-1` に属し、`rocm-libraries/projects/miopen` 系に寄ると読めるが、exact source commit と CLI が `y=int32` route を表現し損ねる位置は未確定 |
| `MIOpenDriver convint8 + out_cast_type=fp32` | driver-side cast path | **accepted but still blocked** | `--out_cast_type fp32` は受理されるが、log 上の output tensor は `dataType = 3` のまま `cast_type: Other` になり、problem key は `...INT8-F_coFP32` へ変わる。GEMM family は `GEMM not supported with casted tensors on this GPU architecture` を返し、`GemmFwd1x1_0_1_int8` も natural / search / forced の全てで practical route にならない | legacy `conv_driver.hpp` の `valid_cast_types` / `DataTypeFromShortString()`、legacy `gemm.cpp` の `problem.IsTensorsCasted()` 分岐、current `conv_driver.hpp` / `convolutionocl.cpp` / `ProblemDescription::IsInt8()`（code_verified）/ `vega64_int8_outcastfp32_nat_1x1_n32_c64_k64_20260318.log` / `vega64_int8_outcastfp32_search_1x1_n32_c64_k64_20260318.log` / `vega64_int8_outcastfp32_force_1x1_n32_c64_k64_20260318.log`（runtime_verified） | **driver / descriptor表現の境界**。direct `y=int32` path の代替ではない | AMD(M) | host 上の installed driver は `miopen-hip 7.2.0-1` 由来で、embedded path は `rocm-libraries/projects/miopen` を示す。したがって provenance は完全な unknown ではないが、exact source commit は未確定 |
| `MIOpenDriver convint8` CLI direct `y=int32` expression | driver option surface | **parser-level blocked** | `convint8 --help` には `in/out/wei_cast_type` は見えるが、direct output dtype を切り替える obvious option は見えない。`--out_data_type int32` は `Long Name: out_data_type Not Found !`、`--out_cast_type int32` は `Invalid value for out_cast_type` で落ちる | `vega64_miopendriver_convint8_help_20260320.txt` / `vega64_miopendriver_out_data_type_int32_20260320.log` / `vega64_miopendriver_out_cast_type_int32_20260320.log`（runtime_verified）/ current `MIOpen/driver/main.cpp`, `MIOpen/driver/conv_driver.hpp`, `miopen-src/driver/dm_convint8.cpp`, `miopen-src/driver/conv_driver.hpp`（code_verified） | **CLI / descriptor assembly 境界**。entrypoint は `ConvDriver<int8_t, int32_t>` でも、visible option surface は direct `y=int32` route をそのまま expose していない | AMD(M) | hidden / undocumented path の有無までは未確認。exact packaging tree も未確定 |
| `MIOpen direct solution query (INT8 -> INT32 output)` | immediate solution query | **query success** | `x=int8, w=int8, y=int32` の descriptor 直叩きで `GetSolutionCount = 2`, `id=89` / `id=85` が返る。`GetSolutionWorkspaceSize(89)` も成功 | `vega64_miopen_int8_solution_probe_20260318.log`（runtime_verified / query-level）/ current `conv_driver.hpp` の `y_type=miopenInt32`、current `convolutionocl.cpp` の int8 validation、current `problem_description.cpp` の dtype key（code_verified） | **solver / descriptor boundary**。current source では `INT8INT8INT32` は cast metadata ではなく実 dtype 問題として表現される | AMD(M) | これは query-level 観測であり、`MIOpenDriver convint8` CLI が同じ path を素直に表現できることまでは意味しない |
...
| `MIOpen standard Find/Forward (INT8 -> INT32 output)` | higher-level C API | **find success** + **runtime success** | `miopenFindConvolutionForwardAlgorithm()` が `GEMM` と `Direct` を返し、`exhaustive=0` では `GEMM` が最速、`exhaustive=1` では `GEMM` のみが返る。`miopenConvolutionForward()` は `algo=GEMM` / `algo=Direct` の両方で成功し、出力は `64` で一致 | `vega64_miopen_int8_find_forward_probe_20260318.log`（runtime_verified） | **API 一般の閉塞ではない**。明示的 `y=int32` descriptor なら standard route でも通る | AMD(M) | `MIOpenDriver convint8` が同じ descriptor を組めない理由は未確定 |
...
- 2026-03-20 の CLI surface follow-up では、installed `MIOpenDriver convint8 --help` に direct output dtype を切り替える obvious option は見えず、`--out_data_type int32` は unknown option、`--out_cast_type int32` は invalid cast token として落ちた
...
- 2026-03-20 の installed package provenance follow-up では、この host の `/opt/rocm/bin/MIOpenDriver` が `miopen-hip 7.2.0-1` に属し、binary / library の埋め込み path が `/usr/src/debug/miopen-hip/rocm-libraries/projects/miopen/...` を指すこと、また `/opt/rocm/include/miopen/version.h` と `libMIOpen.so` が `MIOpen 3.5.1.5b515cf1bca-dirty` を示すことを確認した
...
- CLI surface follow-up により、installed `MIOpenDriver convint8` では direct `out_data_type` knob 自体が visible ではなく、`out_cast_type=int32` も reject されるため、direct `y=int32` route は少なくとも obvious な CLI syntax として expose されていないと読める。
...
- 同時に、installed `/opt/rocm/bin/MIOpenDriver` もこの host では `miopen-hip 7.2.0-1` の packaged binary であり、embedded path は `rocm-libraries/projects/miopen` を指している。したがって installed driver も current public standalone clone と 1:1 に重ねるべきではなく、少なくとも `rocm-libraries/projects/miopen` 系の cast-aware driver family として読む方が自然である。
...
1. **`MIOpenDriver convint8` の output-type path**: installed `/opt/rocm/bin/MIOpenDriver` は host 上で `miopen-hip 7.2.0-1` に属し、embedded path も `rocm-libraries/projects/miopen` を示すが、exact source commit / packaging tree と、local debug build (`miopen-src@f842c61d`) との関係は未確定
2. **`MIOpenDriver convint8` から `y=int32` path をどう踏ませるか**: higher-level C API からの再現には成功した。installed driver では少なくとも obvious な `out_data_type` knob は見えず、`out_cast_type=int32` も reject されるため、同 route をどう表すかは未確認
