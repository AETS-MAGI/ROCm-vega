...
- その過程で注目すべき分岐点が何だったか
- 現在の「一部では使えず、一部では残存している」状態がどう形成されたか

---

## 2. 結論サマリ

**Fact**（公開一次資料・ローカル clone から確認済み）

- 古い時期の changelog には、`gfx900` を追加対象・既定ビルド対象として広げる記述が残っている（`Tensile 4.36.0 for ROCm 5.5.0`、`rocSOLVER 3.26.0 for ROCm 6.2.0`）。
- 2021-12-22 の MIOpen commit `2407d2f556c7635de3f4b3f009681bdd92ba82e2` により、MLIR iGEMM の `gfx900` 経路が AMD 社員（Zhuoran Yin, `zhuoryin@amd.com`）によって除外された（`git blame` 確認済み）。
- この除外コードの直上コメントは `llvm-project-private#389` という非公開 issue を参照している。
- 後年の changelog では、少なくとも hipCUB 4.0.0 で `gfx803` / `gfx900` が既定ビルド対象から外されている。
- 2022-10-05 の MIOpen commit `e5c6ce1b` 由来の現行 tree には、`gfx900` 向け `sramecc-` misreport workaround と Find-db / immediate mode 記述が残っている。
- 現行ソースには MIOpen ASM implicit GEMM、rocBLAS/Tensile の `gfx900` lazy loading、capability-based fallback が残っており、完全消滅ではなく legacy / fallback path の局所的残存という形になっている。

**Interpretation**（上記事実から読める解釈）

- `gfx900` は、ROCm で一度に全面削除されたというより、コンポーネントごとに別々の速度で後退したように見える。
- MLIR 経路除外後も別層の runtime / docs 整備が続いていたことは、単調な一直線の縮退ではなく層差を伴う変遷として読める。

#### Open Question / Limitation

- `llvm-project-private#389` の本文は外部から確認できない。したがって、除外判断の根本原因および経緯の詳細はここから断定できない。
- この除外が「設計判断」か「バグ回避的な一時停止」かは、private issue 本文を確認しない限り確定しない。

**要するに**:
ROCm の GitHub 履歴から見えるのは、「Vega の単純な全面非対応化」という単一イベントではなく、
...
- MIOpen の公開ソースからは「除外された」という事実と「参照先 issue」の存在までは確認できる。
- **しかし、なぜ除外したかの核心説明は public GitHub からは確認できない。**

> `llvm-project-private#389` は非公開であり、本文は外部から確認できない。したがって、ここから言えるのは公開コード側に参照関係と gating の痕跡が存在するという範囲に限られる。

この構造が示すこと:

- `gfx900` の MLIR iGEMM 経路は、単なる自然陳腐化ではなく、AMD 社員によるコミットで除外されたことが `git blame` から確認できる。
- 公開 GitHub 上で理由が完結せず、参照先 issue が非公開のため、後から見る利用者には除外理由が見えない構造になっている。

### 5.2.1 PR #1328 の public 文脈から見えるのは「release/tuning surface からの切り離し」である

`ROCm/MIOpen#1328` の public PR body / comment から確認できること:

- body には次が列挙されている
  - MLIR commit を ROCm 5.1 release branch へ bump
  - `gfx900` を non-xdlops MLIR solver から除外
  - ctest 側でも `gfx900` を無効化
  - `MIOPEN_TEST_VEGA` を `GFX900` / `GFX906` に分割
- Zhuoran Yin 自身の comment では、
  **ROCm 5.1 向け MLIR solver tuning に入る前にこの PR を入れる必要がある**
  と述べている

読み取り:

- public 側だけでも、この PR は単なる一行 guard 追加ではなく、
  **runtime applicability + ctest + tuning/test surface 全体から `gfx900` MLIR non-xdlops を外す整理**
  として提出されている
- ただし、なぜそうする必要があったかの core rationale は依然 `llvm-project-private#389` に閉じている
...
> **Interpretation**: 推測の域を出ないが、この構造は「全面撤去するには影響が大きいため、対応表記や基盤は残しつつ、問題のある solver だけ個別停止した」という経路と一致している。ただしこれは構造的観測に基づく解釈であり、設計意図の断定ではない。
...
### 5.4.1 PR #1231 と issue #1204 の public 文脈は「misreport を userspace で吸収する workaround」を示す

`ROCm/MIOpen#1231` の body は、
internal `SWDEV-303062` に加えて public issue `ROCm/MIOpen#1204` comment を明示参照している。

`ROCm/MIOpen#1204` comment thread で Artem Tamazov が説明している要点:

- HIP runtime が `gfx900` に SRAMECC feature を誤報する
- そのため MIOpen が target name に `sramecc-` を付加する
- しかし `gfx900` には SRAMECC feature 自体がなく、COMGR が invalid target として reject する

さらに PR comment では Jun Liu が、
**legacy ASIC を使う community users に影響するので cherry-pick したい**
と述べている。

読み取り:

- `#1231` は public 文脈からも比較的はっきり理由が読める
- これは「MIOpen が新たに `gfx900` を積極拡張した」というより、
  **lower-layer / driver-runtime 側の target-feature misreport に対する defensive workaround**
  とみるのが自然である
- 同時に、support table の話ではなく **userspace library が lower-layer の異常を吸収する層** が ROCm に存在することも示している
...
### 5.10 Miir 側制約は public rocMLIR で追跡可能

追加調査で、`MiirIsConfigApplicable` の先にある実装が完全非公開ではないことを確認した。

確認先（public）:

- `ROCm/rocMLIR/mlir/tools/rocmlir-lib/Miir.h`
- `ROCm/rocMLIR/mlir/tools/rocmlir-lib/rocmlir-lib.cpp`

観測事実:

- MIOpen 側 `MiirIsConfigApplicable(params)` は `miirLowerTuningParams(handle)` の `MIIR_SUCCESS` 判定のみ
- `miirCreateHandle(arguments)` 側で
  - `parseConvConfig` 失敗 -> `nullptr`
  - `convGenerator.isApplicable()` 失敗 -> `nullptr`
  - `RockEnabled(config)` 失敗 -> `nullptr`
- `RockEnabled` はレイアウト制約 + `bf16` 非対応（`noBF16`）を課す
- `miirLowerTuningParams` は `Applicability` モードで kernel pipeline 実行し、
  成功なら `MIIR_SUCCESS`、失敗なら `MIIR_BUILD_FAILURE`

読み取り:

- これで「MIOpen 前段条件 + Miir 後段条件」の二層構造は public コードで説明できる。
- ただし private `llvm-project-private#389` の本文は依然不明であり、
  2021 年時点の disable 判断の直接根拠は未回収のまま。
...
- これは `gfx900` 直接の分岐点ではないが、ROCm ecosystem が
...
- ROCm における Vega の扱いは、「完全サポート」から「完全削除」に直線的に移ったのではなく、**新しい最適化経路から先に脱落し、旧経路だけがしばらく残る**という legacy 化の典型パターンと一致しているように見える。
- 特に MIOpen の `gfx900` MLIR disable は、少なくとも構造上は compiler backend 側の制約が product code の gating として表面化した事例に見える。private issue 本文が非公開のため、その判断経緯の詳細はここから断定しない。
...
- `rocMLIR` 側 `miirLowerTuningParams` の `MIIR_BUILD_FAILURE` を誘発する具体 case を再現し、
  `MiirIsConfigApplicable` false の実体を runtime と照合する
...
- MIOpen の `gfx900` MLIR disable の根本原因を確定するものではない
- 本文書で言及した事例が ROCm 全体の一般法則として適用できると断定するものではない
