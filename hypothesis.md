...
**総括**: gfx900 は「維持（build）・管理（selection）・補充（fallback）」の3層で説明できる。
...
**確度**: 仮説（次のフェーズは git blame / PR / issue 履歴調査）
...
**確度**: 筋の良い仮説。UDNA の正体そのものは現時点では断言不可（未来の話）
...
- [ ] INT8 非 naive solver の自然選択確認（現状: 全形状で `ConvDirectNaiveConvFwd` のみ）
  → やらないという選択肢もある（FP32 での確認は完了しており、INT8 は追加情報）
...
| コミュニティが保守を主張できるか | OSS層では理論上可能 | 仮説 |
...
| 経路はコミュニティ実装で維持されているか | MLIR 除外は AMD 投入。他経路の出所は未確定 | 要履歴調査 |
| 実際に実機で選ばれているか | FP32 solver 選択は確認済み。INT8非naive は未達 | runtime_verified（部分） |
| UDNA / 再統合との関係 | 意図は未確定、構造は観測できる | 仮説 |

---

## 11. 動的検証追記（2026-03-13）
...
## 12. rocMLIR 追加フェーズ追記（2026-03-13）
...
## 13. issue #389 provenance 確定（2026-03-13）
