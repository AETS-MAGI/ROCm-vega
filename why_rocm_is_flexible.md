...
- コードを変更せずに「out荷成果物から外す」こともでき、逆に「コードに残したまま出荷物を止める」こともできる
...
**Fact として観測された時系列（→ `final_hypothesis.md` §2.5 Mermaid 図 参照）:**

- 2021-12: MLIR iGEMM が gfx900 を除外（solver 単位の撤退）
- 2023: Winograd の perf workaround が投入（FP32 path は継続）
- 2024: Tensile lazy loading fallback が外部 contributor により補修
- 2024: hipCUB が gfx900 を default build から外す（component 単位の後退）
