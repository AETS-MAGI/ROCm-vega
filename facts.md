...
**含意**: gfx900 向けには Find-DB 形式の MIOpen Perf DB が大規模に出荷されており、同一パッケージ内では gfx1100/gfx1200 向けの同形式 Perf DB は観測されない。

**留保**: この比較は、同じチューニングメカニズムを前提とした場合にのみ直接比較として使える。RDNA3/4 が AI-based tuning や CK-based tuning など別の制御方式を主要経路として採用している場合、Perf DB 不在は「手厚くない」ではなく「別方式で運用している」ことを意味しうる。したがって、この事実はまず「gfx900 向けに Find-DB 形式のデータが出荷されている」ことの根拠として読むべきであり、世代間の手厚さ比較は参考値にとどめる。
...
ただし Perf DB 比較については、gfx1100/gfx1200 側が別の tuning mechanism を使っている可能性が残るため、ここから直ちに「gfx900 の方が新世代より手厚い」と一般化することは避けるべきである。少なくともこの節で確定できるのは、gfx900 向けの Find-DB 形式データと rocBLAS 成果物が現に出荷されているという点である。
