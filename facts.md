...
この文書は、今回の調査で「すでに確認できた事実」を時系列と論点別で固定するためのもの。
推測・仮説は最小化し、再実行可能な観測とコード参照に寄せる。

---

## 2. 調査の主目的

- 目的: `-S ConvMlirIgemmFwd` 強制実行時に発生する `MIIR_INVALID_PARAM` の根因を、`miirCreateHandle` 失敗分岐レベルまで確定する。
- 目標状態:
	- runtime 実体（`/opt/rocm` またはローカル debug 置換）で分岐ログを採取できる。
	- `nullptr` 失敗がどの分岐 (`parseConvConfig` / `isApplicable` / `RockEnabled` / `genConvModule` 等) か説明できる。
...
- 主対象:
	- MIOpen runtime: `/opt/rocm/lib/libMIOpen.so.1.0`
	- 参照ソース:
...
- 補助確認:
	- `libMIOpen.so` に `miopen::Miir*` は見える。
	- `miir*` C API シンボルは見えない。
- 結論:
	- 現行 LD_PRELOAD フック方式では MIIR 呼び出しを捕捉できない。
...
- `tools/run_case_with_local_miopen.sh`
	- `LD_LIBRARY_PATH` 先頭にローカル MIOpen を置いてケース実行
...
- `tools/build_rocmlir_local.sh`
	- `BUILD_FAT_LIBROCKCOMPILER=On` など軽量化フラグを導入
...
- ただし最終目的は「強制時にどの分岐で落ちるか」の確定であり、未サポートであることの確認だけでは完了ではない。
