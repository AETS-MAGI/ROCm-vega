...
作成日: 2026-03-17
関連文書: `hypothesis.md`, `facts.md`, `class_map.md`, `support_boundary.md`, `provenance_map.md`, `reveal_hypothesis.md`, `trace_map_static.md`, `trace_map_dynamic.md`, `community_vs_vendor_matrix.md`, `gfx900_history_timeline.md`, `why_rocm_is_flexible.md`
...
| 観測した CK iGEMM 系 path（`ConvCkIgemmFwdV6r1DlopsNchw` 等） | `IsApplicable()` で全件 `not applicable` | 強制実行 15+ ケース全件 `rc=0x3` |
...
| **配布上のサポート** | プリコンパイル済み成果物・Perf DB の出荷 | 残存（出荷成果物を確認） |
...
graph TD
  S["『サポート』という語"]
  A["表のサポート<br/>推奨・QA・優先修正"]
  B["設計上のサポート<br/>capability・fallback"]
  C["配布上のサポート<br/>Perf DB・rocBLAS・firmware"]
  D["運用上のサポート<br/>CI・報告受付・保証"]
  S --> A
  S --> B
  S --> C
  S --> D
...
この読みは、`reveal_hypothesis.md` で整理した
「capability-based selection と fallback は ROCm の中心設計のひとつ」という一般仮説とも整合する。

一方、CK について現時点で強く言えるのは、
観測した CK iGEMM 系 path が `not applicable` だったという範囲に限られる。
Tensile の HIP source fallback は、gfx900 専用実装ではなく
「capability 非対応時の汎用フォールバック」として設計されている。
...
- `2024-03` の Tensile では optimized-logic 不在 arch 向け fallback libraries 方針が再投入された（`#1862` → revert `#1879` → `#1897`）
...
この読みは、`reveal_hypothesis.md` で検討した
ROCm 一般の staged / layered retreat 仮説とも接続する。
これは意図されたものかは観測できないが、少なくとも結果として Layered Retreat として読める。

```mermaid
timeline
    title gfx900 Layered Retreat — 観測された変化の時系列
    2020 : ASM v4r1 dynamic 投入 (AMD contributor)
    2021 : sramecc workaround 追加 (PR-1231)
         : MLIR iGEMM gfx900 除外 (PR-1328, ROCm 5.1)
    2022 : Tensile gfx900-xnack- 追加 (外部 contributor, PR-1595)
    2023 : Winograd perf workaround (PR-1968)
    2024 : Tensile fallback libraries 方針の再投入 (PR-1862 → revert PR-1879 → PR-1897)
    2026 : rocBLAS / Perf DB / firmware 出荷継続を確認
```

補足:
- 以前の調査では `hipCUB` release note に `gfx900` default build 後退の記述を回収しているが、
  current mirror ではその本文を再確認できていない。したがって、この文書では主時系列からは外し、補助観測にとどめる。
...
gfx900 の取り扱いにどう影響するかは、公開側から一部までは確認できる。

少なくとも current `TheRock` では、
`therock_add_amdgpu_target(gfx900 ...)` により `gfx900` 自体は global target として登録される一方、
`EXCLUDE_TARGET_PROJECTS` により `hipBLASLt` / `hipSPARSELt` / `composable_kernel` / `rocWMMA` /
`rocprofiler-compute` など一部 sub-project からは個別に filter される。

したがって build / integration 層でも、gfx900 は「完全に消えた target」ではなく、
**存在は維持されつつ project ごとに selective exclude されうる target** として扱われている。

未確定なのは、この設定が release / packaging policy にどう反映されるかであり、
そこは引き続き別途調査が必要である。
...
単純な残置でも「意図的な維持」でもなく、少なくとも構造上は次の説明が観測と整合する：
...
他の除外のうち、XDLops 系は「gfx900 に存在しない命令セットへの依存」として構造上自然な除外として読める。
一方、CK 系について現時点で確認できるのは、観測した iGEMM path が `not applicable` だったという範囲に限られる。
...
| `community_vs_vendor_matrix.md` | 経路ごとの投入・補修・出荷主体の圧縮表 |
| `gfx900_history_timeline.md` | `gfx900` の主要分岐点を日付つきで整理した canonical timeline |
| `why_rocm_is_flexible.md` | 登録 / 判定 / backend / artifact 分離が Layered Retreat を可能にする理由 |
| `what_can_be_extended.md` | コミュニティが技術的に修正可能な層の一覧と観測根拠 |
| `what_cannot_be_extended.md` | 物理制約・非公開境界・組織的境界による修正困難層の整理 |
| `future_support_paths.md` | 現構造から読める将来経路の含意と再統合への含意 |
| `natural_maintenance_scenarios.md` | 自然維持シナリオ（S1〜S5）：残りやすい層・崩れやすい層・コミュニティの接点 |
| `support_meaning_conclusion.md` | **調査結論ページ**：3問への一文回答・根拠参照・横断的結論 |
| `reveal_hypothesis.md` | ROCm 一般における capability-based selection / staged retreat 仮説の整理 |
