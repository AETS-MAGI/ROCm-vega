...
**Fact**: gfx900_56 の Perf DB 行数（108,129）は gfx1030（111,296）と同規模であり、gfx1100/gfx1200 には同一パッケージ内で同形式の Perf DB が存在しない。

**Interpretation**: MIOpen Perf DB は solver ごとのチューニングパラメータを持つため、その存在は「そのアーキテクチャ向けのチューニング実行がビルド工程に含まれている」ことを示唆する。一方で、gfx1100/1200 に同形式 Perf DB がないことは、別のチューニング制御方式を使っている可能性も含むため、直ちに「gfx900 の方が新世代より手厚い」とまでは言えない。

**Limitation**: Perf DB のビルド工程詳細（自動生成か手動チューニングか、いつ最終更新されたか）は未確認。gfx1100/gfx1200 側が AI-based tuning や CK-based tuning など別方式を主要経路にしている可能性があるため、この比較はまず「gfx900 向けに Find-DB 形式のデータが出荷されている」という事実の根拠として読むべきであり、世代間比較は参考値にとどまる。
...
| 「RDNA3/4 より手厚いか」 | 比較なし | **部分的に yes**（rocBLAS 成果物は gfx900 > gfx1100/gfx1030。Perf DB 比較は tuning mechanism 差に留保が必要） |
